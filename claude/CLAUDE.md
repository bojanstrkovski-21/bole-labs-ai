...
- Push and authentication are a harder line than the rest: default to the user pushing themselves with their own scripts, on any host. Don't reach for `gh auth`, credential-helper, or SSH-agent commands even to check status — if pushing is blocked on auth, say so and stop, don't troubleshoot credentials.
